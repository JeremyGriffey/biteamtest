#-------------------------------------------------------------------------------
# Name:        LaborZendeskRefreshGroups
# Purpose:     Perform refresh of Zendesk Groups from API to data warehouse
# Author:      Chad Rue
# Created:     01/30/2021
#-------------------------------------------------------------------------------
...
def RefreshZenDeskGroups():
    ...
    engine.execute(text('''EXEC dbo.usp_DeleteZDGroupsTable''').execution_options(autocommit=True))
    ...
    while url:
         ...
         dfallcols = pandas.json_normalize(data['groups'])

         #Create dataframe with only columns that will be inserted into staging table
         dfRequiredCols = dfallcols[['id','name','description','default','deleted','created_at','updated_at']]
         ...
         dfRequiredCols = dfRequiredCols.rename(columns = {'id':'GroupID',
                                                            'name':'GroupName',
         ...
         dfRequiredCols.to_sql('StageZDGroups', schema='dbo', con = engine, if_exists = 'append', chunksize = 100, index=False)
    ...
    engine.execute(text('''EXEC dbo.usp_UpdateZDGroupsTable''').execution_options(autocommit=True))

    #Insert new records into reporting table
    engine.execute(text('''EXEC dbo.usp_LoadZDGroupsTable''').execution_options(autocommit=True))

    #Delete data from staging table
    engine.execute(text('''EXEC dbo.usp_DeleteZDGroupsTable''').execution_options(autocommit=True))

def main():
    RefreshZenDeskGroups()
