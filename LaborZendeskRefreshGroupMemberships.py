#-------------------------------------------------------------------------------
# Name:        LaborZendeskRefreshGroupMemberships
# Purpose:     Perform refresh of Zendesk Group Memberships from API to data warehouse
# Author:      Chad Rue
# Created:     01/30/2021
#-------------------------------------------------------------------------------
...
def RefreshZenDeskGroupMemberships():
    ...
    engine.execute(text('''EXEC dbo.usp_DeleteZDGroupMembershipsTable''').execution_options(autocommit=True))
    ...
    while url:
         ...
         dfallcols = pandas.json_normalize(data['group_memberships'])

         #Create dataframe with only columns that will be inserted into staging table
         dfRequiredCols = dfallcols[['id','user_id','group_id','default','created_at','updated_at']]
         ...
         dfRequiredCols = dfRequiredCols.rename(columns = {'id':'GroupMembershipID',
                                                            'user_id':'UserID',
                                                            'group_id':'GroupID',
         ...
         dfRequiredCols.to_sql('StageZDGroupMemberships', schema='dbo', con = engine, if_exists = 'append', chunksize = 100, index=False)
    ...
    engine.execute(text('''EXEC dbo.usp_UpdateZDGroupMembershipsTable''').execution_options(autocommit=True))

    #Insert new records into reporting table
    engine.execute(text('''EXEC dbo.usp_LoadZDGroupMembershipsTable''').execution_options(autocommit=True))

    #Delete data from staging table
    engine.execute(text('''EXEC dbo.usp_DeleteZDGroupMembershipsTable''').execution_options(autocommit=True))

def main():
    RefreshZenDeskGroupMemberships()
